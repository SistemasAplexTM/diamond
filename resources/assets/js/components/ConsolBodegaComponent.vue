                  ...
                  <div class="col-sm-4">
                    <div class="col-sm-12">
                      ...
                      <a
                        ...
                        data-toggle="tooltip"
                        data-placement="top"
                        title="Imprimir instrucciones"
                      >
                        <i class="fal fa-print"></i> Instrucciones
                      </a>
                    </div>
                  </div>
                    ...
                    <div class="col-sm-3">
                      <div class="form-group">
                        ...
                        <button
                          ...
                          type="button"
                          data-toggle="tooltip"
                          title="Agregar estatus a guias"
                          @click="addStatusConsolidado()"
                        >
                          <i class="fal fa-save"></i> Aplicar
                        </button>
                      </div>
                    </div>
                  </div>
                              ...
                              <template v-if="boxes.length > 0" v-for="box in boxes">
                                <li class="list-group-item fist-item">
                                  <div class="row">
                                    <div class="col-sm-4">
                                      <button
                                        class="btn btn-success dim btn-large-dim btn-outline btn-cajas"
                                        data-toggle="tooltip"
                                        title="Bolsa"
                                        data-placement="right"
                                        type="button"
                                        @click="getDataDetail(box.num_bolsa)"
                                      >{{ box.num_bolsa }}</button>
                                    </div>
                                    <div class="col-sm-5">
                                      <div>
                                        <strong>Cajas:</strong>
                                        <span>{{ box.cantidad }}</span>
                                      </div>
                                      <div>
                                        <strong>Kilos:</strong>
                                        <span>{{ box.peso_kl }}</span>
                                        <strong>- Libras:</strong>
                                        <span>{{ box.peso }}</span>
                                      </div>
                                      <div>
                                        <strong>Volumen:</strong>
                                        <span>{{ box.volumen }}</span>
                                      </div>
                                    </div>
                                    <div class="col-sm-3" style="text-align: center;">
                                      ...
                                      <a
                                        ...
                                        title="Trasladar"
                                        data-toggle="tooltip"
                                        style="color:#FFC107;display: none;"
                                        @click="chageBoxModal(box.num_bolsa)"
                                      >
                                      ...
                                      <a
                                        ...
                                        title="Eliminar"
                                        data-toggle="tooltip"
                                        style="color:#E34724;display: none;"
                                        @click="removeBox(box.num_bolsa)"
                                      >
                        ...
                        <div class="col-sm-12">
                          <div class="form-group">
                            ...
                            <button
                              ...
                              data-toggle="tooltip"
                              data-placement="top"
                              title="Guias Disponobles"
                              id="btn_buscarGuias"
                              @click="getModalGuias()"
                            >
                              <i class="fal fa-search-plus"></i> Buscar guias
                            </button>
                          </div>
                        </div>
                      </div>
                    </div>
                    <span class="small pull-right" style="color: orangered;" v-if="msn !== ''">
                      <strong>
                        <i class="fal fa-exclamation-triangle"></i> Atención!
                      </strong>
                      {{ msn }}
                    </span>
                    <h2># Bolsa {{ n_bolsa }}</h2>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel">Guias / Warehouses pendientes por consolidar</h4>
          </div>
          <div class="modal-body">
            <form id="formGuiasConsolidado" name="formGuiasConsolidado" method="POST" action>
              <p>
                Seleccione los documentos que desea ingresar al consolidado y acontinuacion de click en el boton
      ...
      <div class="modal-dialog" style="width: 30%!important;">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title" id="myModalLabel">
              <i class="fal fa-exchange"></i> Trasladar bolsa
            </h4>
          </div>
          <div class="modal-body">
            <div class="row">
              <div class="col-lg-12">
                ...
                <div class="form-group">
                  ...
                  <v-select
                    ...
                    label="consolidado"
                    :filterable="false"
                    :options="consolidados"
                    @search="onSearchConsolidados"
...
export default {
  ...
  mounted() {
    ...
    if (this.documento.central_destino_id != null) {
      this.central_destino_id = {
        id: this.documento.central_destino_id,
        name: this.documento.central_destino
      };
      this.disabled_agencia = true;
    }
    if (this.documento.transporte_id != null) {
      this.transporte_id = {
        id: this.documento.transporte_id,
        nombre: this.documento.transporte
      };
  ...
  methods: {
    ...
    chageBox() {
      ...
      axios
        .get(
          "changeBoxConsolidado/" + me.boxChangeC + "/" + me.consolidado_id.id
        )
    ...
    removeBox(bolsa) {
      ...
      swal({
        title:
          "<div><span style='color: rgb(212, 103, 82);'>Atención!</span><br> La bolsa sera removida de este consolidado y no podra recuperarse despues.</div>",
        ...
        if (result.value) {
          axios
            .get("removeBoxConsolidado/" + bolsa)
            .then(function(response) {
              if (response.data.code === 200) {
                me.getBoxesConsolidado();
        ...
        columns: [
          {
            render: function(data, type, full, meta) {
              ...
              var btn_delete =
                "<a style='float: right;cursor:pointer;''><i class='material-icons'>clear</i></a>";
              if (
                groupGuias != null &&
                groupGuias != "null" &&
                groupGuias != ""
              ...
              if (me.app_type === "courier") {
                if (full.liquidado == 0) {
                  return (
                    full.num_warehouse +
                    '<a style="float: right;cursor:pointer;" class="badge badge-' +
                    color +
                    ' pop" role="button" \n\
                                            data-html="true" \n\
                                            data-toggle="popover" \n\
          ...
          },
          {
            render: function(data, type, full, meta) {
              return (
                '<a data-name="peso2" data-pk="' +
                full.documento_detalle_id +
                '" class="td_edit" data-type="text" data-placement="right" data-title="Peso">' +
                full.peso2 +
                "</a>"
              );
            }
          ...
          { data: "peso", name: "peso" },
          {
            render: function(data, type, full, meta) {
              ...
              return (
                nom_cons +
                ' <a  data-toggle="tooltip" title="Canbiar" class="edit" style="float:right;color:#FFC107;" onclick="showModalShipperConsigneeConsolidado(' +
                full.id +
                ", " +
                full.consignee_id +
                ',\'consignee\')"><i class="material-icons">&#xE254;</i></a> <a onclick="restoreShipperConsignee(' +
                full.id +
                ',\'consignee\')" class="delete" title="Restaurar original" data-toggle="tooltip" style="float:right;color:#2196F3;"><i class="material-icons">cached</i></a>'
              );
            }
          },
          {
            ...
            render: function(data, type, full, meta) {
              ...
              var btn_invoice =
                ...
                "/invoice/" +
                full.documento_detalle_id +
                "' target='blank_' class=''><i class='fal fa-file' style='padding: 3px 2px;'></i> Imprimir invoice</a> ";
              if (me.permissions.pdfLabel) {
                var btn_label =
                  "<a href='" +
                  href_print_label +
                  "' target='blank_' class=''><i class='fal fa-barcode' style='padding: 3px 2px;'></i> Imprimir label</a> ";
              }
              if (me.permissions.deleteDetailConsolidado) {
                var btn_delete =
                  ...
                  "," +
                  false +
                  ")\" class='' style='color:#E34724;'><i class='fal fa-trash' style='padding: 3px 2px;'></i> Eliminar</a> ";
              }
              var btn_group =
    ...
    searchBranch: _.debounce((loading, search, vm) => {
      fetch(`../vueSelectTransportadorMaster/${escape(search)}`).then(res => {
    ...
    search: _.debounce((loading, search, vm) => {
      fetch(`../vueSelect/${escape(search)}`).then(res => {
    ...
    searchConsolidados: _.debounce((loading, search, vm) => {
      fetch(`../../master/vueSelectConsolidados/${escape(search)}`).then(
        res => {
