var objVue = new Vue({
  ...
  methods: {
    ...
    placeShipperConsignee: function (data, table) {
      ...
      if (table === 'shipper' && data != null) {
        me.shipper_data = data
        me.nombreR = data['nombre_full'];
      } else {
        if (table === 'consignee' && data != null) {
    ...
    // IMPRIMIR DOCUMENTOS
    printDocument: async function (url, document) {
      if (document === 'label') {
        if (print_labels != '') {
          await this.print_direct(url, print_labels, "File");
        } else {
      ...
      if (document === 'document') {
        if (print_documents != '') {
          await this.print_direct(url, print_documents, "File");
        } else {
          window.open(url, "_blank");
        }
      }
