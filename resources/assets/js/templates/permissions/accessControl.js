...
var objVue = new Vue({
  ...
  methods: {
    ...
    getPermisionsRole: function(role_id) {
      axios
        .get("accessControl/getPermisionsRole/" + role_id)
        .then(response => {
          ...
          for (var key in this.permisionsRoles) {
            for (var sel in this.selected_permissions) {
              if (
                this.permisionsRoles[key]["c"] ==
                this.selected_permissions[sel].id
              ) {
                this.chk_c.push(this.selected_permissions[sel].id);
                this.updateCheckall("c");
              }
              if (
                this.permisionsRoles[key]["r"] ==
                this.selected_permissions[sel].id
              ) {
    ...
    specialAction: function(module_name) {
      ...
      axios
        .get(
          "accessControl/getSpecialPermisions/" +
            module_name +
            "/" +
            this.role_id.id
        )
    ...
    saveSpecialPermissions: function() {
      let me = this;
      axios
        .post("accessControl/saveSpecialPermissions", {
          role_id: this.role_id.id,
